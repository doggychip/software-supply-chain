...
function harness(t,page='analysis.html',overrides={},query=''){
...
test('financials render without waiting for secondary sources and failed sources do not blank the page',async t=>{
  ...
  const w=harness(t,'analysis.html',{'/api/fundamentals':()=>pending,'/api/quotes':new Error('Synthetic Yahoo outage')});
...
test('FMP outage leaves taxonomy and Yahoo quote context available',async t=>{
  const w=harness(t,'analysis.html',{'/api/financial-data':new Error('Synthetic FMP outage')});await w.dashboardReady;
...
test('future quote timestamps are not displayed as fresh market data',async t=>{
  const w=harness(t,'analysis.html',{'/api/quotes':{quotes:{TEST:{price:10,asOf:Date.now()/1000+3600}}}});await w.dashboardReady;
  assert.equal(w.document.getElementById('quoteCoverage').textContent,'0/2');
  const stale=harness(t,'analysis.html',{'/api/quotes':{quotes:{TEST:{price:10,asOf:Date.now()/1000-8*86400}}}});await stale.dashboardReady;
